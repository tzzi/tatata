...
<body>
	...
	<div class="w3-container" align="center">
		<header class="w3-container w3-pale-red">
			<h1 style="color: white" align="center">R E V I E W &nbsp; B O A
				R D</h1>
		</header>
		<br />
		...
		<table class="w3-table-all w3-hoverable w3-center"
			...
			<tbody>
				<c:forEach items="${matchinglist}" var="a">
					<tr class="w3-center">
						<td width=5%>${a.M_NO}</td>
						<td width=30%><a
							href="/matchingBoard/detail.do?m_no=${a.M_NO}">${a.TITLE}</a></td>
						<td width=15%>${a.WRITER}</td>
						<td width=10%>${a.LEFT_DATE}</td>
						<td width=10%>${a.COUNT}</td>
						<td width=10%>${a.M_LIKE }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
			...
			<tr class="w3-right">
				<td>
					<div class="w3-text-theme w3-right">
						<a href="/matchingBoard/writeform.do" style="margin-right: 3px"><button
							...
							<div class="w3-bar">
								<c:choose>
									<c:when test="${paging.page gt  1}">
										<a
											href="/matchingBoard/matchingreview2.do?page=${paging.page-1}">이전</a>
									</c:when>
								</c:choose>
								...
								<c:forEach var="item" items="${paging}"
									...
									<c:choose>
										...
										<c:otherwise>
											<a
												href="/matchingBoard/matchingreview2.do?page=${status.index }">[${status.index }]</a>
								...
								<c:choose>
									<c:when test="${paging.page lt paging.totalPage }">
										<a
											href="/matchingBoard/matchingreview2.do?page=${paging.page+1 }">다음</a>
									</c:when>
								</c:choose>
								...
								<c:choose>
									<c:when test="${paging.endPage lt paging.totalPage }">
										<a
											href="/matchingBoard/matchingreview2.do?page=${paging.totalPage }">끝</a>
									</c:when>
